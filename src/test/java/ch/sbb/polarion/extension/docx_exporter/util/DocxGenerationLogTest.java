...
class DocxGenerationLogTest {

    private DocxGenerationLog log;

    @BeforeEach
    void setUp() {
        log = new DocxGenerationLog();
    }
    ...
    void shouldRecordTiming() {
        ...
        List<DocxGenerationLog.TimingEntry> entries = log.getTimingEntries();
    ...
    void shouldTrackHierarchicalTimings() {
        try (DocxGenerationLog.Timer outer = log.startTimer("Outer")) {
            try (DocxGenerationLog.Timer inner = log.startTimer("Inner")) {
                // Simulate work
            }
        }
        ...
        DocxGenerationLog.TimingEntry innerEntry = entries.get(0);
        ...
        DocxGenerationLog.TimingEntry outerEntry = entries.get(1);
    ...
    @Test
    void shouldSetDocxMetrics() {
        log.setDocxMetrics(2048, 5);
        log.finish();
    ...
    void shouldGenerateTimingReport() {
        ...
        log.recordTiming("Pandoc conversion", 2000);
        log.recordTiming("DOCX post-processing", 300);
        log.setHtmlSize(50000);
        log.setDocxMetrics(100000, 10);
        log.finish();

        String report = log.generateTimingReport("Test Document");

        assertThat(report)
                .contains("DOCX GENERATION TIMING REPORT")
    ...
    void shouldCategorizeTimingEntries() {
        ...
        log.recordTiming("Pandoc conversion", 500);
        log.recordTiming("DOCX post-processing", 200);
        ...
        assertThat(report)
                ...
                .contains("Pandoc Conversion")
                .contains("DOCX Post-processing")
                .contains("Cover Page");
    }
    ...
    void timerShouldRecordDurationOnClose() throws InterruptedException {
        try (DocxGenerationLog.Timer timer = log.startTimer("Test Stage")) {
            Thread.sleep(10);
        }
    ...
    void timerShouldSupportDetails() {
        try (DocxGenerationLog.Timer timer = log.startTimer("Test Stage").withDetails("extra info")) {
            // Simulate work
        }
    ...
    void timingEntryShouldSupportSimpleConstructor() {
        DocxGenerationLog.TimingEntry entry = new DocxGenerationLog.TimingEntry("Stage", 100, "details");
