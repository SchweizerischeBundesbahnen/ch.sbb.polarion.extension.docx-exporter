...
class HtmlProcessorTest {
    ...
    void keepNotInlinedRelativeUrlsTest() {
        // a relative url and an SVG fragment cannot be loaded by the conversion service, leave them alone
    ...
    void dropExternalCssImportEvenWhenAllowedTest() {
        // the at-rule is never inlined, so pandoc would load it itself, unchecked
    ...
    void dropRelativeCssImportTest() {
        // an at-rule is never embedded, so its target is read by pandoc-service itself, from wherever that
