...
import DocxExportContext from '../../main/resources/webapp/docx-exporter/js/modules/ExportContext.js';
import DocxExportParams from "../../main/resources/webapp/docx-exporter/js/modules/ExportParams.js";
...
describe('ExportContext Class', function () {
    it('URL: #/project/elibrary/wiki/BigDoc', function () {
        ...
        const exportContext = new DocxExportContext({documentType: DocxExportParams.DocumentType.LIVE_DOC, polarionLocationHash: locationHash});

        expect(exportContext.documentType).to.equal(DocxExportParams.DocumentType.LIVE_DOC);
    ...
    it('URL: #/wiki/TestLiveReport', function () {
        ...
        const exportContext = new DocxExportContext({documentType: DocxExportParams.DocumentType.LIVE_REPORT, polarionLocationHash: locationHash});

        expect(exportContext.documentType).to.equal(DocxExportParams.DocumentType.LIVE_REPORT);
        expect(exportContext.projectId).to.be.null;
    ...
    it('URL: #/project/elibrary/testrun?id=elibrary_20231026-163136654', function () {
        ...
        expect(exportContext.documentType).to.equal(DocxExportParams.DocumentType.TEST_RUN);
        expect(exportContext.projectId).to.equal('elibrary');
    ...
    it('URL: #/project/elibrary/collection?id=144', function () {
        ...
        const exportContext = new DocxExportContext({documentType: DocxExportParams.DocumentType.BASELINE_COLLECTION, polarionLocationHash: locationHash});

        expect(exportContext.documentType).to.equal(DocxExportParams.DocumentType.BASELINE_COLLECTION);
        expect(exportContext.projectId).to.equal('elibrary');
        expect(exportContext.locationPath).to.be.undefined;
        expect(exportContext.baselineRevision).to.be.undefined;
        expect(exportContext.revision).to.be.undefined;
