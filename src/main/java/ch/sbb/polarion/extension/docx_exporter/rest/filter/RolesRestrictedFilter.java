package ch.sbb.polarion.extension.docx_exporter.rest.filter;
 ...
 * the request with {@code 403 Forbidden} when the current user is not authorized to export DOCX for that project.
...
public class RolesRestrictedFilter implements ContainerRequestFilter {
    ...
    RolesRestrictedFilter(@NotNull DocxExporterPolarionService polarionService, @NotNull ISecurityService securityService, @NotNull ResourceInfo resourceInfo) {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        ...
        if (!isAuthorized(projectId)) {
            ...
            throw new ForbiddenException("Current user is not allowed to export DOCX for this project");
        }
    }
