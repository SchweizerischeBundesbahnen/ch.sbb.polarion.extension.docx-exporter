...
public class DocumentPropertiesPaneStatusProvider extends ConfigurationStatusProvider {
    ...
    public @NotNull ConfigurationStatus getStatus(@NotNull Context context) {
        ...
        if (content == null) {
            ...
            return getConfigurationStatus(DOCUMENT_PROPERTIES_PANE, content, "<extension id=\"docx-exporter\".*/>", new ConfigurationStatus(DOCUMENT_PROPERTIES_PANE, Status.WARNING, "Not configured neither for global scope nor for current project"));
        } else {
            return getConfigurationStatus(DOCUMENT_PROPERTIES_PANE, content, "<extension id=\"docx-exporter\".*/>", new ConfigurationStatus(DOCUMENT_PROPERTIES_PANE, Status.WARNING, "Not configured for current project"));
        }
    }
}
