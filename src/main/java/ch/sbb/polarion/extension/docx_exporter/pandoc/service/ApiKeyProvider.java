...
public class ApiKeyProvider {
     ...
     *         user friendly so the reason survives the catch-all of the conversion paths and reaches
     *         the export dialog, where the person who can fix the configuration reads it
     */
    public @Nullable String getApiKey() {
        ...
        try {
            ...
            throw new UserFriendlyRuntimeException(String.format(
                    "Could not read the pandoc API key from the Polarion secret '%s' (%s)", secretName, e.getClass().getName()));
        }
        if (apiKey == null || apiKey.isBlank()) {
            throw new UserFriendlyRuntimeException(String.format("The Polarion secret '%s', configured as the pandoc API key, is empty or does not exist", secretName));
        }
