...
/**
 * DOCX generation profiler that extends the generic ExecutionProfiler
 * with DOCX-specific metrics and category breakdown.
 */
public class DocxGenerationLog extends ExecutionProfiler {
    ...
    public void setDocxMetrics(long sizeBytes, int pages) {
        this.docxSizeBytes = sizeBytes;
        this.pageCount = pages;
    }

    /**
     * Generates a DOCX-specific timing report with additional metrics.
     *
    ...
    protected void appendHeader(StringBuilder report, @Nullable String title) {
        ...
        report.append("DOCX GENERATION TIMING REPORT").append(System.lineSeparator());
        report.append("=".repeat(80)).append(System.lineSeparator());
    ...
    private void appendSummaryStatistics(StringBuilder report) {
        ...
        if (docxSizeBytes > 0) {
            report.append(String.format("  DOCX Size:      %s%n", formatSize(docxSizeBytes)));
        }
        if (pageCount > 0) {
    ...
    private static final Map<String, List<String>> CATEGORY_PATTERNS = Map.of(
            "Pandoc Conversion", List.of("pandoc", "conversion"),
            "DOCX Post-processing", List.of("docx", "post-process", "template", "style"),
