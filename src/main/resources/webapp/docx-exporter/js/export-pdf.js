const ExportDocx = {
    ...
    stylePackageSelected: function (stylePackage) {
        ...
        DocxExportCommon.displayIf("page-width-validation", stylePackage.exposePageWidthValidation);
    },
    ...
    loadPdf: function (projectId, locationPath, baselineRevision, revision) {
        ...
        DocxExportCommon.asyncConvertPdf(request, result => {
            if (result.warning) {
            ...
            errorResponse.text().then(errorJson => {
                ...
                $("#export-error").append("Error occurred during DOCX generation" + (errorMessage ? ":<br>" + errorMessage : ""));
            });
        });
    },
    ...
    loadDocx: function (projectId, locationPath, baselineRevision, revision) {
        ...
        let fileName = document.getElementById("docx-filename").value;
        if (!fileName) {
            fileName = document.getElementById("docx-filename").dataset.default;
        }
        if (fileName && !fileName.endsWith(".docx")) {
            fileName += ".docx";
        }
    ...
    actionInProgress: function (inProgress) {
        if (inProgress) {
            //disable components
            $("#" + $("#export-docx").parent().parent().attr("id") + " :input").attr("disabled", true);
            //show loading icon
            $("#export-docx-progress").show();
        } else {
            //enable components
            $("#" + $("#export-docx").parent().parent().attr("id") + " :input").attr("disabled", false);
            //hide loading icon
            $("#export-docx-progress").hide();
        }
    },
    ...
    validatePdf: function (projectId, locationPath, baselineRevision, revision) {
        ...
        SbbCommon.callAsync({
            method: "POST",
            url: "/polarion/docx-exporter/rest/internal/validate?max-results=6",
            contentType: "application/json",
            responseType: "json",
            body: request,
            onOk: (responseText, request) => {
