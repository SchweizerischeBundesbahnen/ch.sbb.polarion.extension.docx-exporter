...
const BulkDocxExporter = {
    ...
    renderBulkExportItems: function (bulkExportWidget) {
        if (bulkExportWidget) {
            ...
            bulkExportWidget.querySelectorAll('input[type="checkbox"]:not(.export-all):checked').forEach((selectedCheckbox) => {
                ...
                nameSpan.innerText = this.getDocumentType(selectedCheckbox.dataset["type"]) === DocxExportParams.DocumentType.BASELINE_COLLECTION ? selectedCheckbox.dataset["name"] : this.getSpace(selectedCheckbox) + selectedCheckbox.dataset["id"];
    ...
    startNextItemExport: function () {
        ...
        if (this.exportParams && pausedItems && pausedItems.length > 0) {
            ...
            if (documentType === DocxExportParams.DocumentType.TEST_RUN) {
                this.exportParams["urlQueryParameters"] = {id: documentId};
                DocxExportCommon.downloadTestRunAttachments(this.exportParams.projectId, documentId, this.exportParams.revision, this.exportParams.attachmentsFilter);
            } else if (documentType === DocxExportParams.DocumentType.BASELINE_COLLECTION) {
    ...
    getDocumentType: function (itemType) {
        switch (itemType) {
            case "Module": return DocxExportParams.DocumentType.LIVE_DOC;
            case "RichPage": return DocxExportParams.DocumentType.LIVE_REPORT;
            case "TestRun": return DocxExportParams.DocumentType.TEST_RUN;
            case "BaselineCollection": return DocxExportParams.DocumentType.BASELINE_COLLECTION;
            default: return "";
        }
    },
