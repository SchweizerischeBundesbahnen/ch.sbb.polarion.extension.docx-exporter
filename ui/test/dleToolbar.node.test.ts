...
// single dleEditorHead script instead of a script plus an inline DocxExporterStarter.injectToolbar
// call. All it does is get starter.js loaded and then make that call.
//
...
describe('dle-toolbar.js injector', () => {
  ...
  it('reuses an already loaded starter instead of loading it twice', async () => {
    ...
    globals().DocxExporterStarter = starterStub();
  ...
  it('logs instead of throwing when starter.js loads without defining the global', async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      expect.stringContaining('starter.js loaded but DocxExporterStarter is not defined'),
    );
  });
