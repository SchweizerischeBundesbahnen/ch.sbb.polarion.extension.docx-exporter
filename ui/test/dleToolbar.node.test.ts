...
const SELF_URL = 'http://localhost/polarion/docx-exporter/js/dle-toolbar.js';
...
describe('dle-toolbar.js injector', () => {
  ...
  const load = async (selfUrl = SELF_URL): Promise<void> => {
    ...
    await import('../../src/main/resources/webapp/docx-exporter/js/dle-toolbar.js');
    await flushPromises();
  };
  ...
  it('describes the button on the script tag, which is what the engine installs from', async () => {
    ...
    expect(dataset.marker).toBe('docx-exporter');
    expect(dataset.title).toBe('Export to DOCX');
    expect(dataset.icon).toContain('/polarion/ria/images/dle/operations/actionMsWordRoundtrip16.svg');
    expect(dataset.permissionUrl).toBe('/polarion/docx-exporter/rest/internal/permissions/export');
  });

  it('opens the export dialog on click', async () => {
    await load();

    const onclick = engineScript()!.dataset.onclick!;
    expect(onclick).toContain(`${APP_BASE}assets/export-popup.js`);
    expect(onclick).toContain('openExportPopup()');
