...
type Content = { documentNameTemplate: string };
...
async function useApi(): Promise<ReturnType<typeof useNamedSettings<Content>>> {
  ...
  function Probe() {
    api = useNamedSettings<Content>('filename-template');
    return null;
  }
  render(<Probe />);
...
describe('useNamedSettings', () => {
  it('renames a configuration by posting the new name', async () => {
    const fetchMock = installFetchMock([
      { method: 'POST', match: /\/settings\/filename-template\/names\/Old/, json: {} },
    ]);
    ...
    expect(String(url)).toBe(
      '/polarion/docx-exporter/rest/internal/settings/filename-template/names/Old?scope=project%2Felibrary%2F',
    );
    expect(init!.body).toBe('New name');
  });

  it('creates and deletes a named configuration', async () => {
    // Not used by the filename template page (a single setting), but the same hook serves the pages
    // still on JSP - style packages, localization, webhooks - which do have named configurations.
    const fetchMock = installFetchMock([
      { method: 'PUT', match: /\/settings\/filename-template\/names\/Compact\/content/, json: {} },
      { method: 'DELETE', match: /\/settings\/filename-template\/names\/Compact/, json: {} },
    ]);
    const api = await useApi();

    await api.createConfiguration('Compact', '');
    await api.deleteConfiguration('Compact', '');

    expect(fetchMock.mock.calls.map(([, init]) => init?.method)).toEqual(['PUT', 'DELETE']);
    expect(String(fetchMock.mock.calls[0][0])).toContain('/names/Compact/content?scope=');
  });

  it('asks for a specific revision when one is given', async () => {
    const fetchMock = installFetchMock([
      { method: 'GET', match: /\/settings\/filename-template\/names\/Default\/content/, json: { css: '' } },
    ]);
