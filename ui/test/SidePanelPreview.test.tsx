...
// document it remembers, and the editor hash it writes for the one that was picked. That hash is what
...
const panelRoutes = (): Route[] => [
  ...
  { method: 'POST', match: /\/export-filename/, respond: () => new Response('Doc.docx') },
...
describe('the side panel development harness', () => {
  ...
  it('preselects the document remembered for this project', async () => {
    document.cookie = 'docx-exporter-dev-document=elibrary%7CSpecs%2FRequirements; path=/';
  ...
  it('ignores a document remembered for another project', async () => {
    document.cookie = 'docx-exporter-dev-document=other%7CSpecs%2FRequirements; path=/';
  ...
  it('ignores a remembered document the project no longer offers', async () => {
    document.cookie = 'docx-exporter-dev-document=elibrary%7CGone%2FDeleted; path=/';
