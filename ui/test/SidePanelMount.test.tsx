...
function host(): HTMLElement {
  ...
  element.id = `docx-exporter-panel-${hosts.length}`;
  element.className = 'docx-exporter form-wrapper';
...
describe('mounting the side panel', () => {
  ...
  it('says it is loading rather than showing an empty pane', async () => {
    ...
    // A button-sized spinner alone reads as a stuck glyph in an empty pane, so this is 48px, centered in a
    // column, with the message under it.
    const element = host();
    ...
    // Centered in a column, the message under the spinner rather than beside it
