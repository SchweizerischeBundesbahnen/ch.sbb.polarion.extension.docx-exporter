...
// DocxExporterStarter.injectToolbar(), and that has to keep working. Since react-sbb-polarion 2.0.0 it does
...
describe('starter.js compatibility shim', () => {
  ...
  const starter = () => globals().DocxExporterStarter as { injectToolbar: (params?: unknown) => void };
  ...
  it('injects through dle-toolbar.js, resolved from its own URL', async () => {
    ...
    expect(injectorScript()!.src).toContain('/polarion/docx-exporter/js/dle-toolbar.js');
  });
