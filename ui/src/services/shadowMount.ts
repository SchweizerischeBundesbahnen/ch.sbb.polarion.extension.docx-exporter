...
interface ShadowMountOptions {
   ...
   *  extension's own panel CSS expects, e.g. `docx-exporter form-wrapper sbb-ui`). */
 ...
 * version. docx-exporter and pdf-exporter are usually installed side by side and both put a panel
 * there, so this is not a hypothetical clash. Plain CSS is global by selector; a shadow root gives
 * true, two-way encapsulation: react-sbb-polarion's stylesheet (injected as a `<style>` INSIDE the
 * shadow) styles only this panel and cannot leak out, and the page's / other extensions' styles cannot
 * leak in. The SearchableDropdown popup portal is shadow-aware (it appends into this root via
 * `getRootNode()`), so the dropdown is styled and isolated too.
 *
 * `?inline` gives the built stylesheet as a string; extension-specific panel CSS is bundled the same
