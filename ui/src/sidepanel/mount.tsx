 ...
 * form-extension fragment (webapp/docx-exporter/html/sidePanelContent.html) dynamically imports this
 * module and calls `mountSidePanel("#docx-exporter-panel")`.
 *
 * The panel is mounted inside a **shadow root** on that fragment div, so its styles are fully
 * encapsulated on the shared editor page (see services/shadowMount.ts). The wrapper classes the panel CSS
 * expects (`docx-exporter form-wrapper`) plus the token scope (`sbb-ui`) are reproduced on the inner
 * container, and `side-panel.css` is bundled (via `?inline`) and injected into the shadow alongside
...
export function mountSidePanel(selector: string, deps?: SidePanelDependencies): Root | undefined {
  ...
  if (!host) {
    console.error(`docx-exporter: side panel mount target "${selector}" not found.`);
    return undefined;
  }
