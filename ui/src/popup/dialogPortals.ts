 ...
 * "Custom orientation" or switching comments on mounts a `SearchableSelect` that creates its portal right
 * then.
 *
 ...
 * that deliberate "always on top" value stopped being reachable. pdf-exporter hit this first and carries the
 * same helper; strictdoc breaks the same way the moment it upgrades.
 *
 * So the fix belongs in RSP, which knows both the portal and the dialog. Until it is there, this is the one
 * place in this extension that needs it - the side panel has no dialog.
 */
