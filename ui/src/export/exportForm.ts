 ...
 * differs is that a setting the style package expresses as "null means take what the reference template
 * says" is two fields here - the checkbox that switches it on and the value it carries - so unticking a
 * box does not throw away what the user picked before ticking it again. The Style Packages
 * administration page models its own form the same way, for the same reason.
 */
...
export function toExportForm(content: StylePackageSettings, context: ExportFormContext = {}): ExportForm {
  ...
  return {
    ...
    language: documentLanguage ?? content.language ?? DEFAULT_LANGUAGE,
